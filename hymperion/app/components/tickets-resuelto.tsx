...
export default function NetworkAlerts() {
  ...
  const fetchRiskData = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetch('http://localhost:5000/tickets/resueltos')
      if (!response.ok) {
  ...
  const updateTicketStatus = async (id: number, newStatus: string,) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(result.message) // Imprime el mensaje del backend en la consola
        fetchRiskData() // Actualiza la lista de datos después de cambiar el estado
        setSelectedRisk(null)
      } else {
        console.error('Error al actualizar el estado del ticket:', response.status)
      }
    } catch (error) {
  ...
  const handleRedirect = () => {
    ...
    if (currentPath === '/dashboard') {
      router.push('/dashboard/resueltos')  // Redirige a la opción 1
    } else if (currentPath === '/dashboard/resueltos') {
      router.push('/dashboard')  // Redirige a la opción 2
    } else {
